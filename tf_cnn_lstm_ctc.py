...
def get_next_batch(batch_size=128):
    ...
    img_dir_path = "D:\\Program\\PythonWorkspace\\ocr_tensorflow_cnn\\srcImage\\predict-data\\"
    for i in range(BATCH_SIZE):
        for parent, dirnames, filenames in os.walk(img_dir_path):
            index = random.randint(0, len(filenames) - 1)
            image, text = obj.gen_custom_image(img_dir_path, filenames[index])
            # cv2.imshow('image', image)
            # cv2.waitKey(0)
            # np.transpose 矩阵转置 (32*256,) => (32,256) => (256,32)
            inputs[i, :] = np.transpose(image.reshape((OUTPUT_SHAPE[0], OUTPUT_SHAPE[1])))
...
def train():
    ...
    def do_batch():
        ...
        if steps > 0 and steps % REPORT_STEPS == 0:
            do_report()
            # 随机图片识别模型
            # save_path = saver.save(session, "D:\\Program\\PythonWorkspace\\ocr_tensorflow_cnn\\model\\ocr.model", global_step=steps)
            # ODB数字粒识别模型
            save_path = saver.save(session, "D:\\Program\\PythonWorkspace\\ocr_tensorflow_cnn\\odb-model\\ocr.model",
                                   global_step=steps)
            # print(save_path)
        return b_cost, steps
    ...
    with tf.Session() as session:
        ...
        for curr_epoch in range(num_epochs):
            ...
            for batch in range(BATCHES):
                ...
                if curr_epoch > 0 and curr_epoch % BATCHES == 0:
...
def predict():
    ...
    with tf.Session() as session:
        session.run(init)
        saver = tf.train.Saver(tf.global_variables(), max_to_keep=100)
        # saver.restore(session, "D:\\Program\\PythonWorkspace\\ocr_tensorflow_cnn\\model\\ocr.model-1700")
        saver.restore(session, "D:\\Program\\PythonWorkspace\\ocr_tensorflow_cnn\\odb-model\\ocr.model-20000")
...
if __name__ == '__main__':
    inputs, sparse_targets, seq_len = get_next_batch(BATCH_SIZE)
