...
class gen_id_card(object):
    ...
    def gen_image(self, flag):
        text, vec = self.random_text(flag)
    ...
    # 获取自定义的图片，返回标签和图片元素数据 --- RJ.CHOI
    def gen_custom_image(self, img_dir_path, img_name):
        # 获取训练图片
        text = img_name.split('.')

        # image_src = Image.open(img_dir_path + img_name)  # image is a PIL image
        image_src = mpimg.imread(img_dir_path + img_name)
        dstImage = cv2.resize(image_src, (256, 32), 3);

        res = np.array(dstImage)  # array is a numpy array
...
if __name__ == '__main__':
    ...
    #img_dir_path = "D:\\Program\\PythonWorkspace\\ocr_tensorflow_cnn\\srcImage\\target-2\\"
    img_dir_path = "D:\\Program\\PythonWorkspace\\ocr_tensorflow_cnn\\srcImage\\"
    img_name = "084.jpg"
    image_data, label = genObj.gen_custom_image(img_dir_path, img_name)
    print("label:", label)
